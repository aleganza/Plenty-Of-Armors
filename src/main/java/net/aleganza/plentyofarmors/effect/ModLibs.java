            ...
            for (BlockPos blockPos2 : BlockPos.iterate(blockPos.add(-i, -1, -i), blockPos.add(i, -1, i))) {
                if (blockPos2.isWithinDistance(entity.getPos(), (double)i)) {
                    ...
                    if (blockState2.isAir()) {
                        ...
                        if (blockState3.isOf(Blocks.WATER) && blockState.canPlaceAt(world, blockPos2) && world.canPlace(blockState, blockPos2, ShapeContext.absent())) {
        ...
        if (entity.isOnGround()) {
            ...
            while(var7.hasNext()) {
                ...
                if (blockPos2.isWithinDistance(entity.getPos(), (double)i)) {
                    ...
                    if (blockState2.isAir()) {
                        BlockState blockState3 = world.getBlockState(blockPos2);
                        if (blockState3.isOf(Blocks.LAVA) && blockState.canPlaceAt(world, blockPos2) && world.canPlace(blockState, blockPos2, ShapeContext.absent())) {
                            world.setBlockState(blockPos2, blockState);
