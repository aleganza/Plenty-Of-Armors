...
public class ModArmorStardusite1 extends ArmorItem {
    ...
    private void addStatusEffectForMaterial(PlayerEntity player, ArmorMaterial mapArmorMaterial, StatusEffectInstance mapStatusEffect) {
        ...
        if(hasCorrectArmorOn(mapArmorMaterial, player) && !hasPlayerEffect) {
            player.addStatusEffect(new StatusEffectInstance(ModEffects.FIRE_WALKER,
                    effectDuration, amplifier, false, false, false));
        }

        // effect repeat issue fix
        if (player.getActiveStatusEffects().containsKey(ModEffects.FIRE_WALKER)) {
