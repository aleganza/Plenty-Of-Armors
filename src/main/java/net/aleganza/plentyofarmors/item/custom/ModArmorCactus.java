...
public class ModArmorCactus extends ArmorItem {
    ...
    public ModArmorCactus(RegistryEntry<ArmorMaterial> material, ArmorItem.Type slot, Settings settings) {
    ...
    private void evaluateArmorEffects(PlayerEntity player) {
        for (Map.Entry<RegistryEntry<ArmorMaterial>, StatusEffectInstance> entry : MATERIAL_TO_EFFECT_MAP.entrySet()) {
            RegistryEntry<ArmorMaterial> mapArmorMaterial = ModArmorMaterials.CACTUS;
    ...
    private void addStatusEffectForMaterial(PlayerEntity player, RegistryEntry<ArmorMaterial> mapArmorMaterial, StatusEffectInstance mapStatusEffect) {
        ...
        if(hasCorrectArmorOn(mapArmorMaterial, player) && !hasPlayerEffect) {
            player.addStatusEffect(new StatusEffectInstance(ModEffects.THORNS,
                    effectDuration, amplifier, false, false, false));
        }

        // effect repeat issue fix
        if (player.getActiveStatusEffects().containsKey(ModEffects.THORNS)) {
